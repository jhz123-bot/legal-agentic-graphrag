...
law_pool = [
    ...
    ('最高人民法院关于审理劳动争议案件适用法律问题的解释（一）', '总则', '劳动争议'),
]


def article_no(idx: int) -> str:
    return f'第{800 + idx}条'
...
for i in range(1, 321):
    ...
    content = (
        ...
        f'本条样本编号{i}，人民法院应结合证据链完整性、损失结果、过错程度与责任基础综合裁判。\n'
        f'页码:{i}\n'
    )
    statutes.append({
...
case_types = ['盗窃案件', '合同违约案件', '租赁纠纷案件', '侵权责任案件', '公司纠纷案件', '劳动争议案件', '买卖合同案件', '民间借贷案件', '股东出资纠纷案件', '产品责任案件']
courts = ['某市中级人民法院', '某区人民法院', '某省高级人民法院', '某铁路运输法院', '某知识产权法院']
...
faq_topics = ['盗窃罪', '诈骗罪', '合同违约', '租赁纠纷', '侵权责任', '公司纠纷', '劳动争议', '买卖合同', '民间借贷', '股东出资']
faqs = []
...
with (root / 'statutes' / 'statutes_raw_round4.jsonl').open('w', encoding='utf-8') as f:
    for row in statutes:
        f.write(json.dumps(row, ensure_ascii=False) + '\n')

with (root / 'cases' / 'cases_raw_round4.jsonl').open('w', encoding='utf-8') as f:
    for row in cases:
        f.write(json.dumps(row, ensure_ascii=False) + '\n')

with (root / 'faqs' / 'faqs_raw_round4.jsonl').open('w', encoding='utf-8') as f:
    for row in faqs:
        f.write(json.dumps(row, ensure_ascii=False) + '\n')
